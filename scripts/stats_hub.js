...
function drawHist(){
  if(typeof document==='undefined')return;if(!_st.histData.length)return;const c=document.getElementById('histCanvas'),ctx=c.getContext('2d');const dpr=typeof window!=='undefined'?window.devicePixelRatio||1:1;c.width=c.offsetWidth*dpr;c.height=300*dpr;ctx.scale(dpr,dpr);const W=c.offsetWidth,H=300;ctx.clearRect(0,0,W,H);
  const bins=+document.getElementById('binSlider').value;const mn=Math.min(..._st.histData),mx=Math.max(..._st.histData);const bw=(mx-mn)/bins||1;const counts=new Array(bins).fill(0);_st.histData.forEach(v=>{let b=Math.min(Math.floor((v-mn)/bw),bins-1);counts[b]++;});const maxC=Math.max(...counts);
...
// ===================== BOXPLOT =====================
function drawBox(){
  if(typeof document==='undefined')return;const type=document.getElementById('boxPreset').value;let data;
  ...
  const c=document.getElementById('boxCanvas'),ctx=c.getContext('2d');const dpr=typeof window!=='undefined'?window.devicePixelRatio||1:1;c.width=c.offsetWidth*dpr;c.height=260*dpr;ctx.scale(dpr,dpr);const W=c.offsetWidth,H=260;ctx.clearRect(0,0,W,H);
...
// ===================== NORMAL =====================
function drawNorm(){
  if(typeof document==='undefined')return;const mu=+document.getElementById('muSlider').value,sig=+document.getElementById('sigSlider').value,region=document.getElementById('normRegion').value;
  ...
  const c=document.getElementById('normCanvas'),ctx=c.getContext('2d');const dpr=typeof window!=='undefined'?window.devicePixelRatio||1:1;c.width=c.offsetWidth*dpr;c.height=340*dpr;ctx.scale(dpr,dpr);const W=c.offsetWidth,H=340;ctx.clearRect(0,0,W,H);
...
// ===================== COMPARE =====================
function drawComp(){
  if(typeof document==='undefined')return;const muA=+document.getElementById('muAS').value,sigA=+document.getElementById('sigAS').value,muB=+document.getElementById('muBS').value,sigB=+document.getElementById('sigBS').value;
  ...
  const c=document.getElementById('compCanvas'),ctx=c.getContext('2d');const dpr=typeof window!=='undefined'?window.devicePixelRatio||1:1;c.width=c.offsetWidth*dpr;c.height=320*dpr;ctx.scale(dpr,dpr);const W=c.offsetWidth,H=320;ctx.clearRect(0,0,W,H);
...
function showAnswerHistory(probId){
  ...
  var rows=hist.slice(-5).reverse().map(function(h){
    return '<div class="ah-row"><span class="ah-when">'+_ago(h.time)+'</span><span class="ah-val">'+_esc(String(h.val))+'</span><span class="ah-status '+(h.correct?'ah-ok':'ah-wrong')+'">'+(h.correct?'✓':'✗')+'</span></div>';
  }).join('');
...
function vizTemplate(unit){
  if(unit===2){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">Z-Score Explorer</div><div class="viz-desc">Adjust x, mu, and sigma to see left and right tail areas in real time.</div><canvas id="zCanvas" height="320" role="img" aria-label="Z-Score Explorer: normal distribution with shaded tail areas"></canvas><div class="controls"><div class="cg"><span class="cl">x <span class="sv-val" id="u2XVal">72</span></span><input type="range" id="u2X" min="20" max="120" value="72" step="1" aria-label="x value, range 20 to 120" oninput="drawZscore()"></div><div class="cg"><span class="cl">mu <span class="sv-val" id="u2MuVal">70</span></span><input type="range" id="u2Mu" min="30" max="100" value="70" step="1" aria-label="mu (population mean), range 30 to 100" oninput="drawZscore()"></div><div class="cg"><span class="cl">sigma <span class="sv-val" id="u2SigVal">10</span></span><input type="range" id="u2Sig" min="1" max="25" value="10" step="1" aria-label="sigma (standard deviation), range 1 to 25" oninput="drawZscore()"></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u2Z">-</div><div class="sl">z-score</div></div><div class="sc"><div class="sv sv-amber" id="u2Left">-</div><div class="sl">percentile</div></div><div class="sc"><div class="sv sv-pink" id="u2Right">-</div><div class="sl">right area</div></div></div></div><div class="explain"><h3>Formula</h3><p>z converts raw score to standard-deviation units.</p><div class="formula">z = (x - mu) / sigma</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===3){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">Interactive Scatterplot</div><div class="viz-desc">Click to add points. Presets load example patterns for positive, negative, or no correlation.</div><canvas id="scatterCanvas" height="330" role="img" aria-label="Interactive scatterplot with regression line"></canvas><div class="controls"><div class="cg"><span class="cl">Presets</span><div class="viz-btn-group"><button type="button" data-u3preset="positive" onclick="setScatterPreset('positive')">Positive</button><button type="button" data-u3preset="negative" onclick="setScatterPreset('negative')">Negative</button><button type="button" data-u3preset="none" onclick="setScatterPreset('none')">None</button><button type="button" onclick="clearScatter()">Clear</button></div></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u3Eq">-</div><div class="sl">regression line</div></div><div class="sc"><div class="sv sv-amber" id="u3R">-</div><div class="sl">r</div></div><div class="sc"><div class="sv sv-pink" id="u3R2">-</div><div class="sl">r^2</div></div><div class="sc"><div class="sv sv-purple" id="u3N">-</div><div class="sl">n</div></div></div></div><div class="explain"><h3>Least Squares Line</h3><p>Line updates from clicked points and minimizes squared residuals.</p><div class="formula" id="u3Formula">yhat = a + bx</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===4){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">Sampling Simulator</div><div class="viz-desc">Population has 100 dots in 4 groups. Choose a method and compare sample composition.</div><canvas id="samplingCanvas" height="330" role="img" aria-label="Sampling simulator showing population and sample groups"></canvas><div class="controls"><div class="cg"><span class="cl">Method</span><div class="viz-btn-group"><button type="button" data-u4method="srs" onclick="runSampling('srs')">SRS</button><button type="button" data-u4method="stratified" onclick="runSampling('stratified')">Stratified</button><button type="button" data-u4method="cluster" onclick="runSampling('cluster')">Cluster</button><button type="button" data-u4method="systematic" onclick="runSampling('systematic')">Systematic</button><button type="button" onclick="resetSampling()">Reset</button></div></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u4Pop">-</div><div class="sl">population composition</div></div><div class="sc"><div class="sv sv-amber" id="u4Sample">-</div><div class="sl">sample composition</div></div><div class="sc"><div class="sv sv-pink" id="u4Method">-</div><div class="sl">method</div></div><div class="sc"><div class="sv sv-purple" id="u4Size">-</div><div class="sl">sample size</div></div></div></div><div class="explain"><h3>Sampling Design Tradeoffs</h3><p>Method choice can bias representation even when sample size is unchanged.</p><div class="formula">Compare sample mix to population mix.</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===5){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">Venn Diagram Probability</div><div class="viz-desc">Set P(A), P(B), and P(A and B) and watch conditional probabilities update.</div><canvas id="vennCanvas" height="320" role="img" aria-label="Venn diagram showing probability regions A, B and their intersection"></canvas><div class="controls"><div class="cg"><span class="cl">P(A) <span class="sv-val" id="u5PAVal">0.55</span></span><input type="range" id="u5PA" min="0" max="1" step="0.01" value="0.55" aria-label="P(A) probability, range 0 to 1" oninput="drawVenn()"></div><div class="cg"><span class="cl">P(B) <span class="sv-val" id="u5PBVal">0.45</span></span><input type="range" id="u5PB" min="0" max="1" step="0.01" value="0.45" aria-label="P(B) probability, range 0 to 1" oninput="drawVenn()"></div><div class="cg"><span class="cl">P(A and B) <span class="sv-val" id="u5PABVal">0.20</span></span><input type="range" id="u5PAB" min="0" max="0.45" step="0.01" value="0.20" aria-label="P(A and B) joint probability, range 0 to 0.45" oninput="drawVenn()"></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u5Union">-</div><div class="sl">P(A union B)</div></div><div class="sc"><div class="sv sv-amber" id="u5AcondB">-</div><div class="sl">P(A|B)</div></div><div class="sc"><div class="sv sv-pink" id="u5BcondA">-</div><div class="sl">P(B|A)</div></div><div class="sc"><div class="sv sv-purple" id="u5Comp">-</div><div class="sl">P(A')</div></div></div></div><div class="explain"><h3>Independence</h3><p id="u5Independence">Independent? Compare P(A and B) to P(A)P(B).</p><div class="formula">P(A union B) = P(A) + P(B) - P(A and B)</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===6){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">Binomial Explorer</div><div class="viz-desc">PMF bars for X~Bin(n,p), with cumulative probability P(X<=k).</div><canvas id="binomCanvas" height="330" role="img" aria-label="Binomial distribution probability mass function bar chart"></canvas><div class="controls"><div class="cg"><span class="cl">n <span class="sv-val" id="u6NVal">12</span></span><input type="range" id="u6N" min="1" max="40" value="12" step="1" aria-label="n number of trials, range 1 to 40" oninput="drawBinom()"></div><div class="cg"><span class="cl">p <span class="sv-val" id="u6PVal">0.40</span></span><input type="range" id="u6P" min="0.01" max="0.99" value="0.40" step="0.01" aria-label="p probability of success, range 0.01 to 0.99" oninput="drawBinom()"></div><div class="cg"><span class="cl">k <span class="sv-val" id="u6KVal">4</span></span><input type="range" id="u6K" min="0" max="12" value="4" step="1" aria-label="k target count, range 0 to 12" oninput="drawBinom()"></div><div class="cg"><span class="cl">Overlay</span><label class="viz-inline"><input type="checkbox" id="u6Normal" onchange="drawBinom()"> Normal approximation</label></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u6Mean">-</div><div class="sl">E(X)=np</div></div><div class="sc"><div class="sv sv-amber" id="u6SD">-</div><div class="sl">SD(X)=sqrt(npq)</div></div><div class="sc"><div class="sv sv-pink" id="u6Cum">-</div><div class="sl">P(X<=k)</div></div></div></div><div class="explain"><h3>Binomial PMF</h3><p>Each bar is P(X=k) for one count of successes.</p><div class="formula">P(X=k) = C(n,k)p^k(1-p)^(n-k)</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===7){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">CLT Simulator</div><div class="viz-desc">Top: population. Bottom: sampling distribution of xbar built from repeated samples.</div><div class="viz-split-label">Population</div><canvas id="cltPopCanvas" height="180" role="img" aria-label="Population distribution histogram"></canvas><div class="viz-split-label">Sampling distribution of xbar</div><canvas id="cltSampleCanvas" height="180" role="img" aria-label="Sampling distribution of x-bar"></canvas><div class="controls"><div class="cg"><span class="cl">Population shape</span><select id="u7Shape" onchange="setCLTShape(this.value)"><option value="uniform">Uniform</option><option value="right">Right-skewed</option><option value="bimodal">Bimodal</option></select></div><div class="cg"><span class="cl">Sample size n</span><select id="u7N" onchange="setCLTN(+this.value)"><option value="1">1</option><option value="5" selected>5</option><option value="10">10</option><option value="30">30</option><option value="50">50</option></select></div><div class="cg"><span class="cl">Sampling</span><div class="viz-btn-group"><button type="button" onclick="cltDraw(1)">Draw 1 Sample</button><button type="button" onclick="cltDraw(100)">Draw 100 Samples</button><button type="button" onclick="cltReset()">Reset</button></div></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u7PopMu">-</div><div class="sl">population mu</div></div><div class="sc"><div class="sv sv-amber" id="u7MeanXbar">-</div><div class="sl">mean of xbars</div></div><div class="sc"><div class="sv sv-pink" id="u7SE">-</div><div class="sl">SE obs/theory</div></div><div class="sc"><div class="sv sv-purple" id="u7Count">-</div><div class="sl">number of xbars</div></div></div></div><div class="explain"><h3>Central Limit Theorem</h3><p>As n increases, xbar becomes more normal and less variable.</p><div class="formula">SE(xbar) = sigma / sqrt(n)</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===8){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">CI Coverage Simulator</div><div class="viz-desc">Run 100 intervals and count captures of the true proportion.</div><canvas id="covCanvas" height="360" role="img" aria-label="Confidence interval coverage simulation showing 100 intervals"></canvas><div class="controls"><div class="cg"><span class="cl">True p <span class="sv-val" id="u8PVal">0.50</span></span><input type="range" id="u8P" min="0.10" max="0.90" step="0.01" value="0.50" aria-label="true proportion p, range 0.10 to 0.90" oninput="drawCoverage()"></div><div class="cg"><span class="cl">Confidence</span><select id="u8CL" onchange="drawCoverage()"><option value="0.90">90%</option><option value="0.95" selected>95%</option><option value="0.99">99%</option></select></div><div class="cg"><span class="cl">Sample size n <span class="sv-val" id="u8NVal">100</span></span><input type="range" id="u8N" min="20" max="500" step="1" value="100" aria-label="sample size n, range 20 to 500" oninput="drawCoverage()"></div><div class="cg"><span class="cl">Simulation</span><button type="button" onclick="runCoverage()">Run 100 Simulations</button></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u8Capture">-</div><div class="sl">capture count</div></div><div class="sc"><div class="sv sv-amber" id="u8Actual">-</div><div class="sl">actual rate</div></div><div class="sc"><div class="sv sv-pink" id="u8Nominal">-</div><div class="sl">nominal rate</div></div></div></div><div class="explain"><h3>Coverage Meaning</h3><p>Confidence level is long-run capture rate of the method.</p><div class="formula">phat +/- z* sqrt(phat(1-phat)/n)</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===9){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">P-Value Visualizer</div><div class="viz-desc">Select tail type, test statistic, and alpha to compare p-value against critical boundaries.</div><canvas id="pvalCanvas" height="330" role="img" aria-label="P-value visualizer showing normal distribution with shaded tail"></canvas><div class="controls"><div class="cg"><span class="cl">Tail</span><select id="u9Tail" onchange="drawPValue()"><option value="left">Left-tail</option><option value="right" selected>Right-tail</option><option value="two">Two-tail</option></select></div><div class="cg"><span class="cl">z <span class="sv-val" id="u9ZVal">1.20</span></span><input type="range" id="u9Z" min="-4" max="4" step="0.01" value="1.20" aria-label="z test statistic, range -4 to 4" oninput="drawPValue()"></div><div class="cg"><span class="cl">alpha</span><select id="u9Alpha" onchange="drawPValue()"><option value="0.01">0.01</option><option value="0.05" selected>0.05</option><option value="0.10">0.10</option></select></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u9Stat">-</div><div class="sl">test statistic</div></div><div class="sc"><div class="sv sv-amber" id="u9P">-</div><div class="sl">p-value</div></div><div class="sc"><div class="sv sv-pink" id="u9Decision">-</div><div class="sl">decision</div></div></div></div><div class="explain"><h3>Decision Rule</h3><p id="u9CritText">Reject H0 when p-value <= alpha.</p><div class="formula">Compare p-value to alpha.</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===10){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">Chi-Square Distribution Explorer</div><div class="viz-desc">Use df and chi-square sliders to see the right-tail p-value and critical cutoffs.</div><canvas id="chiCanvas" height="330" role="img" aria-label="Chi-square distribution with right-tail p-value shading"></canvas><div class="controls"><div class="cg"><span class="cl">df <span class="sv-val" id="u10DfVal">4</span></span><input type="range" id="u10Df" min="1" max="15" value="4" step="1" aria-label="degrees of freedom, range 1 to 15" oninput="drawChiSq()"></div><div class="cg"><span class="cl">chi^2 value <span class="sv-val" id="u10XVal">6.0</span></span><input type="range" id="u10X" min="0" max="30" value="6.0" step="0.1" aria-label="chi-square value, range 0 to 30" oninput="drawChiSq()"></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u10DfStat">-</div><div class="sl">df</div></div><div class="sc"><div class="sv sv-amber" id="u10XStat">-</div><div class="sl">chi^2</div></div><div class="sc"><div class="sv sv-pink" id="u10PStat">-</div><div class="sl">p-value</div></div></div></div><div class="explain"><h3>Right Tail</h3><p id="u10CritText">Critical values shown for alpha=0.05 and 0.01.</p><div class="formula">p-value = P(Chi-square_df >= observed)</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===12){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">ANOVA Visualizer</div><div class="viz-desc">Adjust group means and within-group spread to see F-statistic update in real time.</div><canvas id="anovaCanvas" height="320" role="img" aria-label="ANOVA group distributions showing between and within group variance"></canvas><div class="controls"><div class="cg"><span class="cl">Group 1 Mean <span class="sv-val" id="u12M1Val">8</span></span><input type="range" id="u12M1" min="1" max="20" value="8" step="0.5" aria-label="group 1 mean, range 1 to 20" oninput="drawAnova()"></div><div class="cg"><span class="cl">Group 2 Mean <span class="sv-val" id="u12M2Val">10</span></span><input type="range" id="u12M2" min="1" max="20" value="10" step="0.5" aria-label="group 2 mean, range 1 to 20" oninput="drawAnova()"></div><div class="cg"><span class="cl">Group 3 Mean <span class="sv-val" id="u12M3Val">12</span></span><input type="range" id="u12M3" min="1" max="20" value="12" step="0.5" aria-label="group 3 mean, range 1 to 20" oninput="drawAnova()"></div><div class="cg"><span class="cl">Within-group SD <span class="sv-val" id="u12SDVal">2</span></span><input type="range" id="u12SD" min="0.5" max="6" value="2" step="0.1" aria-label="within-group standard deviation, range 0.5 to 6" oninput="drawAnova()"></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u12SSB">-</div><div class="sl">SSB</div></div><div class="sc"><div class="sv sv-pink" id="u12SSW">-</div><div class="sl">SSW</div></div><div class="sc"><div class="sv sv-amber" id="u12F">-</div><div class="sl">F-stat</div></div><div class="sc"><div class="sv sv-green" id="u12Eta">-</div><div class="sl">η²</div></div></div></div><div class="explain"><h3>ANOVA Formula</h3><p>F tests if between-group variance exceeds within-group variance.</p><div class="formula">F = MSB/MSW = (SSB/df_B)/(SSW/df_W)</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===13){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">Rank-Based vs Parametric Test</div><div class="viz-desc">See how rankings make nonparametric tests robust to skewness and outliers.</div><canvas id="nonparamCanvas" height="320" role="img" aria-label="Rank-based vs parametric test distribution comparison"></canvas><div class="controls"><div class="cg"><span class="cl">Distribution Shape</span><select id="u13Shape" onchange="drawNonparam()"><option value="normal">Normal</option><option value="skew">Right-skewed</option><option value="outlier">With Outlier</option></select></div><div class="cg"><span class="cl">Group Shift <span class="sv-val" id="u13ShiftVal">2</span></span><input type="range" id="u13Shift" min="0" max="6" value="2" step="0.5" aria-label="group shift amount, range 0 to 6" oninput="drawNonparam()"></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u13RankDiff">-</div><div class="sl">rank diff</div></div><div class="sc"><div class="sv sv-amber" id="u13MeanDiff">-</div><div class="sl">mean diff</div></div></div></div><div class="explain"><h3>Ranking Data</h3><p>Ranks replace actual values, removing sensitivity to extreme values and skewness.</p><div class="formula">rₛ = 1 − 6Σdᵢ²/n(n²−1)</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===14){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">Bayesian Updating</div><div class="viz-desc">Watch how prior, likelihood, and posterior interact as you adjust evidence strength.</div><canvas id="bayesCanvas" height="320" role="img" aria-label="Bayesian updating showing prior, likelihood and posterior distributions"></canvas><div class="controls"><div class="cg"><span class="cl">Prior Strength <span class="sv-val" id="u14PriorVal">5</span></span><input type="range" id="u14Prior" min="1" max="20" value="5" step="1" aria-label="prior strength, range 1 to 20" oninput="drawBayes()"></div><div class="cg"><span class="cl">Observed p̂ <span class="sv-val" id="u14DataVal">0.70</span></span><input type="range" id="u14Data" min="0.01" max="0.99" value="0.70" step="0.01" aria-label="observed proportion, range 0.01 to 0.99" oninput="drawBayes()"></div><div class="cg"><span class="cl">Sample Size <span class="sv-val" id="u14NVal">20</span></span><input type="range" id="u14N" min="1" max="100" value="20" step="1" aria-label="sample size, range 1 to 100" oninput="drawBayes()"></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u14PostMean">-</div><div class="sl">posterior mean</div></div><div class="sc"><div class="sv sv-amber" id="u14PostSD">-</div><div class="sl">posterior SD</div></div><div class="sc"><div class="sv sv-pink" id="u14CI">-</div><div class="sl">95% CI</div></div></div></div><div class="explain"><h3>Bayes Theorem</h3><p>Prior belief updated by data evidence yields the posterior.</p><div class="formula">P(θ|data) ∝ P(data|θ) · P(θ)</div></div></div>`+vizPracticeBtn(unit);
  }
  if(unit===11){
    return `<div class="sub-panel active"><div class="viz-box"><div class="viz-title">Regression Output Interpreter</div><div class="viz-desc">Choose a dataset preset and hover highlighted values for quick interpretation tips.</div><canvas id="regCanvas" height="340" role="img" aria-label="Regression output table with highlighted coefficient values"></canvas><div class="controls"><div class="cg"><span class="cl">Preset dataset</span><select id="u11Preset" onchange="drawRegOut()"><option value="study">Study Hours vs Score</option><option value="ads">Ad Spend vs Sales</option><option value="house">Home Size vs Price</option></select></div></div><div class="stats-row"><div class="sc"><div class="sv sv-cyan" id="u11Slope">-</div><div class="sl">slope</div></div><div class="sc"><div class="sv sv-amber" id="u11SE">-</div><div class="sl">SE</div></div><div class="sc"><div class="sv sv-pink" id="u11T">-</div><div class="sl">t-stat</div></div><div class="sc"><div class="sv sv-purple" id="u11P">-</div><div class="sl">p-value</div></div><div class="sc"><div class="sv sv-green" id="u11R2">-</div><div class="sl">r^2</div></div></div><div id="u11Tip" class="viz-tip">Hover a highlighted value for explanation.</div></div><div class="explain"><h3>Inference Formulas</h3><p>Use slope inference to test whether the linear relationship differs from zero.</p><div class="formula">t = b1 / SE(b1) &nbsp;&nbsp; CI: b1 +/- t* SE(b1)</div></div></div>`+vizPracticeBtn(unit);
  }
  return '';
}
...
function buildPomoHistory(){
  ...
  dates.forEach(function(d){
    ...
    html+='<div class="pomo-history-row"><span>'+_esc(label)+'</span><span>'+r.count+' session'+(r.count===1?'':'s')+' ('+r.mins+' min)</span></div>';
  });
  el.innerHTML=html;
}
...
function buildRecentActivity(){
  ...
  todayArr.forEach(function(a){
    ...
    html+='<div class="ra-row"><span class="ra-id">#'+_esc(String(a.id))+'</span><span class="ra-unit">'+unitName+'</span><span class="ra-status '+statusCls+'">'+statusText+'</span></div>';
  });
  el.innerHTML=html;
}
...
function filterByTopic(topic){
  ...
  if(label)label.textContent='Topic: '+topic;
  document.querySelectorAll('.pc').forEach(function(el){
    var topicSpan=el.querySelector('.pc-topic');
    var t=topicSpan?topicSpan.textContent:'';
    el.style.display=(t===topic)?'':'none';
  });
}
...
function rateProblem(probId,rating){
  ...
  var rd=typeof document!=='undefined'?document.getElementById('prd-'+probId):null;
